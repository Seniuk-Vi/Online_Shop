...
public class ShowHomePageCommand implements Command {
    ...
    private List<Product> selection(HttpServletRequest req, Connection con, int maxPrice, int minPrice) {
        ...
        int noOfPages = rowsCount / 8;
        if (noOfPages * rowsCount % 8 > 0) {
        ...
        List<Product> table = productDao.findAllWithSelect(con, 8, (currentPage - 1) * 8, category, minPriceS, maxPriceS, sort, way, search);
        return table;
    }
}
